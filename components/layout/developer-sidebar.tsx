...
import { developerNavigationConfig } from "@/config/navigation";
import { useState } from "react";
import { useAuth } from "@/components/auth";

// Index route is a prefix of every sub-route, so it must match exactly.
const ROOT_HREF = "/developer/dashboard";

function NavGroup({
  items,
  onItemClick,
}: {
  items: (typeof developerNavigationConfig)[0]["items"];
  onItemClick?: () => void;
}) {
  ...
  return (
    <>
      <div className="px-2 pb-5 pt-1">
        ...
        <span className="mt-3 inline-flex items-center rounded-md bg-primary/15 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-primary">
          Developer
        </span>
      </div>
      ...
      <div className="px-1 pb-2">
        ...
        <nav>
          <NavGroup items={developerNavigationConfig[0].items} />
        </nav>
      </div>

      <div className="mt-auto px-1">
        <div className="border-t border-sidebar-border pt-2">
          <NavGroup items={developerNavigationConfig[1].items} />
        </div>
        <div className="mt-3">
          <UserProfile />
        </div>
...
export function DeveloperMobileSidebar() {
